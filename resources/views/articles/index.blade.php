...
<body class="bg-gray-100 text-gray-800">
    ...
    <header class="{{ $topic === 'laravel' ? 'bg-orange-500' : 'bg-gray-900' }} px-10 py-5 sticky top-0 z-50 flex items-center justify-between">
        ...
        <div class="flex flex-col md:flex-row gap-6 items-start">
            ...
            <aside class="w-full md:w-72 md:shrink-0 md:sticky md:top-25">
                <div class="bg-white rounded-lg p-5 shadow-sm">
                    <h2 class="text-sm font-bold mb-4 pb-2 border-b-2 {{ $topic === 'laravel' ? 'border-orange-500' : 'border-gray-900' }}">
                        人気記事ランキング
                    </h2>
                    @foreach ($popular as $i => $article)
                        <div class="flex gap-3 items-start mb-3 last:mb-0">
                            <span class="text-lg font-bold {{ $topic === 'laravel' ? 'text-orange-500' : 'text-gray-900' }} w-5 shrink-0">
                                {{ $i + 1 }}
                            </span>
                            <div>
                                <a href="{{ $article->url }}" target="_blank" rel="noopener"
                                    class="text-sm text-gray-900 {{ $topic === 'laravel' ? 'hover:text-orange-500' : 'hover:text-gray-600' }} leading-snug block">
                                    {{ $article->title }}
                                </a>
